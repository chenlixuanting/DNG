...
public class TrainSpeedDaoImpl extends BaseDaoImpl<TrainSpeed> implements TrainSpeedDao {
    ...
    public List<TrainSpeed> listTrainSpeedByRoadId(String roadId) {
        String hql = "from cn.guet.navigator.web.pojo.TrainSpeed as t where t.roadId=:roadId";
        try {
    ...
    public List<String> countAllDistinctRoadId() {
        String hql = "select distinct s.roadId from cn.guet.navigator.web.pojo.TrainSpeed as s";
        try {
            Query query = getCurrentSession().createQuery(hql);
            return query.list();
        } catch (Exception e) {
    ...
    public List<TrainSpeed> listTrainSpeedBySpecifyTimeAndRoadId(Timestamp startTime, Timestamp endTime, String roadId) {
        String hql = "from cn.guet.navigator.web.pojo.TrainSpeed as s where s.roadId=:roadId and s.startTime=:startTime and s.endTime=:endTime and s.speed!=0";
        try {
            Query query = getCurrentSession().createQuery(hql);
            query.setParameter("roadId",roadId);
